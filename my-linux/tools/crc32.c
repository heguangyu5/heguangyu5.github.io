...
{
	if (argc != 2) {
		fprintf(stderr, "usage: %s bzImage.pad\n", argv[0]);
		return 1;
	}
	...
	if (fd < 0) {
		fprintf(stderr, "open bzImage.pad error\n");
		return 1;
	}
	fstat(fd, &sb);
	printf("bzImage.pad size = %ld\n", sb.st_size);
	file = mmap(NULL, sb.st_size, PROT_READ, MAP_SHARED, fd, 0);
	printf("crc32 bzImage.pad = %x\n", partial_crc32(file, sb.st_size, crc));
